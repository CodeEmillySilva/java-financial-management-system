...
public class TelaVisualizarGastos extends JFrame {
	...
	public TelaVisualizarGastos(Usuario usuarioMenu) {
		
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		this.setSize(DEFAULT_WIDTH, DEFAULT_HEIGHT);
		
		List<Gasto> gastos = gastoBD.buscarGastos(usuarioMenu);
		...
		Object[][] dados = new Object [gastos.size()][6];
		
		if (!gastos.isEmpty()) {
			
			for (int i=0; i<gastos.size(); i++) {
				Gasto parametro = gastos.get(i);
		...
		if (gastos.isEmpty()) {
			...
			+ "nenhum gasto foi cadastrado.","Planilha vazia",JOptionPane.WARNING_MESSAGE);
