...
public class TelaFiltrarMes extends JFrame{
	
	private static final int DEFAULT_WIDTH=1000;
	private static final int DEFAULT_HEIGHT=1000;

	public TelaFiltrarMes(Usuario usuarioMenu) {
		
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		this.setSize(DEFAULT_WIDTH, DEFAULT_HEIGHT);
		
		JLabel titulo = new JLabel("ESCOLHA UM MÊS:");
		titulo.setAlignmentX(CENTER_ALIGNMENT);
		titulo.setFont(new Font("Arial", Font.BOLD,24));
		
		String [] opcoes = {"Escolha uma opção","01","02","03","04","05","06","07","08","09","10","11","12"};
		...
		juncao.setBorder(new EmptyBorder(400,300,450,300));
		juncao.add(titulo);
		...
		botao2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {		
				...
				String mesEscolhido = (String) escolha.getSelectedItem();
				
				if(mesEscolhido.equals("Escolha uma opção")){
					JOptionPane.showMessageDialog(null,"Por favor, escolha um mês.","Erro",JOptionPane.WARNING_MESSAGE);
					return;
				}
				
				dispose();
				new TelaVerPorMes(mesEscolhido,usuarioMenu);
