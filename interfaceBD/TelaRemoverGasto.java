...
public class TelaRemoverGasto extends JFrame {
	...
	public TelaRemoverGasto(Usuario usuarioMenu) {
		...
		JLabel titulo = new JLabel("REMOVER GASTO");
		...
		if (gastosExistentes.isEmpty()) {
			...
			+ "não será possível remover nenhum item","Planilha vazia",JOptionPane.WARNING_MESSAGE);
			dispose();
		...
		botao2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				boolean check = gastoBD.removerGasto(usuarioMenu, gastoSelecionado);
				
				if (check) {
					
					String[] bottons = {"Menu","Vizualizar","Sair"};
					int escolha = JOptionPane.showOptionDialog(null, "Remoção foi efetuada com sucesso, como deseja prosseguir?",
					"Remoção realizada!",
					...
					String mensagem = "Ocorreu um erro na remoção do item, por favor, tente novamente,";
					telaErro(mensagem);
					return;
				}
		...
		JLabel orientacao = new JLabel("Escolha o elemento que deseja remover:");
